...
test('SQL: only database administrators can assign staff roles',async()=>{const db=await setup();try{await db.exec(`insert into account_roles(user_id,role) values('${alice}','owner');`);for(const role of ['anon','authenticated','service_role']){const {rows}=await db.query(`select has_table_privilege('${role}','public.account_roles','INSERT') as can_insert,has_table_privilege('${role}','public.account_roles','UPDATE') as can_update,has_table_privilege('${role}','public.account_roles','SELECT') as can_read`);assert.equal(rows[0].can_insert,false);assert.equal(rows[0].can_update,false);assert.equal(rows[0].can_read,role==='service_role');}await db.exec('set role service_role');assert.equal((await db.query('select role from account_roles')).rows[0].role,'owner');}finally{await db.close();}});
