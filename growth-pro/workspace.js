...
function authMode(mode='signin'){
 signup=mode==='signup';recovering=mode==='recovery';
 $('auth-title').textContent=recovering?'Set a new password':signup?'Your next chapter.':'Welcome back.';
 $('login-submit').textContent=recovering?'Save new password':signup?'Create account':'Sign in';
 $('signup').textContent=signup?'Already have an account?':'Create an account';
 $('signup').hidden=recovering;$('reset').hidden=recovering;$('email').readOnly=recovering;
 $('password').autocomplete=signup||recovering?'new-password':'current-password';
}
...
function lock(){
 accessEpoch++;clearTimeout(expiryTimer);state=null;authMode();
 $('app').hidden=true;$('access').hidden=false;$('login-form').hidden=false;$('mfa-form').hidden=true;$('billing-gate').hidden=true;
 $('billing-account').textContent='';$('conversation').replaceChildren();$('profile-form').reset();$('question').value='';$('mfa-code').value='';$('mfa-qr').removeAttribute('src');$('mfa-qr').hidden=true;
...
function scheduleExpiry(){
 clearTimeout(expiryTimer);if(!state)return;if(state.membership.access_type==='staff'&&['owner','admin'].includes(state.membership.role))return;
...
async function gate(){
 ...
 try{
  ...
  $('mfa-form').hidden=true;const account=await api('/api/account');$('billing-account').textContent='Signed in as '+account.email+'.';state=await api('/api/workspace');$('access').hidden=true;$('app').hidden=false;message('');render();scheduleExpiry();
 ...
 }catch(e){if(e.code==='SESSION_CHANGED')return;if(e.code==='SUBSCRIPTION_REQUIRED'){lock();$('login-form').hidden=true;$('mfa-form').hidden=true;$('billing-gate').hidden=false;message('Your membership is not active. Your saved work is retained.');try{const a=await api('/api/account');$('billing-account').textContent='Signed in as '+a.email+(a.paidUntil?'. Last paid access: '+new Date(a.paidUntil).toLocaleDateString('en-ZA'):'. No verified Pro payment yet.');}catch{}}else if(e.code==='SIGN_IN'){await auth.auth.signOut({scope:'local'});lock();$('login-form').hidden=false;message(e.message);}else{message(e.message,$('access').hidden);}}
}
...
function chat(text,role){const el=document.createElement('div');el.className='message '+role;if(role==='assistant')window.ZionAnswer.render(el,text);else el.textContent=text;$('conversation').append(el);el.scrollIntoView({block:'nearest'});}
...
window.addEventListener('appinstalled',()=>{installPrompt=null;$('install').hidden=true;message('Growth Pro is installed.',true);});
...
try{if(!createClient)throw new Error('Sign-in could not load. Reload the page when your connection is available.');const response=await fetch(base+'/api/config',{cache:'no-store',signal:AbortSignal.timeout(75000)});const config=await response.json();if(!response.ok||!config.configured)throw new Error('Account services are being connected. Please return shortly.');$('login-submit').disabled=false;$('signup').disabled=false;$('reset').disabled=false;message('');auth=createClient(config.authUrl,config.authKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});auth.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_OUT')lock();if(event==='PASSWORD_RECOVERY'){lock();authMode('recovery');$('email').value=session?.user?.email||'';$('auth-title').textContent='Set a new password';$('login-form').hidden=false;$('login-submit').textContent='Save new password';}});await gate();async function checkMembership(){if(!state)return;scheduleExpiry();if(!state)return;try{const latest=await api('/api/workspace');if(state){state.membership=latest.membership;scheduleExpiry();}}catch(e){if(state)message('Membership verification is temporarily unavailable. Reconnect to continue.',true);}}setInterval(checkMembership,60000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)checkMembership();});}catch(e){message(e.message);$('login-submit').disabled=true;$('signup').disabled=true;$('reset').disabled=true;}
